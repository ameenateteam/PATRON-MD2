...
const setupLinkDetection = (sock) => {
    ...
    sock.ev.on('group.participants.update', async (event) => {
        ...
        try {
            ...
            // If actor is not in the event, assume it's the first admin (not ideal but a fallback)
            let actor = event.actor || event.author;
            
            // If still no actor, try to guess (this is a workaround)
            if (!actor) {
                const admins = groupMetadata.participants.filter(p => p.admin === 'admin' || p.admin === 'superadmin');
                actor = admins.length > 0 ? admins[0].id : user; // Fallback to user if no admin found (not ideal)
            }
